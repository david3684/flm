...
DIT_USE_COMPILE=TRUE python -u -m main \
  ...
  wandb.name=lm1b_scratch_small_gamma_alpha_flow_ratio_0.75 \
  model=small \
  training.loss_type=shortcut_consistency \
  ...
  sampling.noise_removal=consistency \
  algo.scale_loss=False \
  algo.shortcut_type=$SHORTCUT_TYPE \
